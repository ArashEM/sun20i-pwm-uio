...
int uio_get_mem_offset(struct uio_info_t* info, int map_num)
{
	int ret;
	char filename[64];
	info->maps[map_num].off = UIO_INVALID_OFF;
	sprintf(filename, "/sys/class/uio/uio%d/maps/map%d/offset",
		info->uio_num, map_num);
	FILE* file = fopen(filename,"r");
	if (!file) return -1;
	ret = fscanf(file,"0x%lx",&info->maps[map_num].off);
	fclose(file);
	if (ret<0) return -2;
	return 0;
}
...
void uio_show_device(struct uio_info_t *info)
{
	char dev_name[16];
...
void uio_show_addr(struct uio_info_t *info, int map_num)
{
	char dev_name[16];
	struct uio_map_t *map = &info->maps[map_num];
	sprintf(dev_name,"uio%d",info->uio_num);
	printf("%s: addr: 0x%lx, size: %d, offset: 0x%lx\n",
		dev_name, map->addr, map->size, map->off);
}

unsigned long uio_offset(struct uio_info_t *info, int map_num)
{
	struct uio_map_t *map = &info->maps[map_num];
	return map->off;
}

int uio_size(struct uio_info_t *info, int map_num)
{
	struct uio_map_t *map = &info->maps[map_num];
	return map->size;
}
